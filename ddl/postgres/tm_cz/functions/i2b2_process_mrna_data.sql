...
BEGIN
  ...
  -- a unique top-level node for mRNA data
	...
	sqlText := ' alter table ' || partitionName || ' add constraint ' || partitionIndx || '_probeset_id_fk foreign key (probeset_id) references deapp.de_mrna_annotation(probeset_id);';
	raise notice 'sqlText= %', sqlText;
	execute sqlText;
	stepCt := stepCt + 1;
