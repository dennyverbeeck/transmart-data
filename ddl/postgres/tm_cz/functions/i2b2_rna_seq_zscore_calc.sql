...
-- calculate zscore
	...
	sqlText := ' alter table ' || partitionName || ' add constraint ' || partitionIndx || '_probeset_id_fk foreign key (probeset_id) references deapp.de_rnaseq_annotation(transcript_id);';
	raise notice 'sqlText= %', sqlText;
	execute sqlText;
	stepCt := stepCt + 1;
